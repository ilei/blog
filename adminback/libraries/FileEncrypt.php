...
class FileEncrypt {
	...
	public function  encryptFile($s_file, $d_file, $key = '', $iter = 32){
		$this->setIter($iter);
		if(file_exists($s_file) && file_exists($d_file)){
			$source = file_get_contents($s_file);	
			file_put_contents($d_file, $this->encrypt($source, $key));
			return true;
		}
	}
	
	public function  decryptFile($s_file, $d_file, $key = '', $iter = 32){
		$this->setIter($iter);
		if(file_exists($s_file) && file_exists($d_file)){
			$source = file_get_contents($s_file);	
			file_put_contents($d_file, $this->decrypt($source, $key));
			return true;
		}
	}
